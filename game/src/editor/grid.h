...
public:
	Grid(QWidget *parent = 0, const QString &name = QString("Untitled"), uint32 width = 0, uint32 height = 0);
	...
	void SetMLOn(bool value)
		{ _ml_on = value; updateGL(); }

	void SetULOn(bool value)
		{ _ul_on = value; updateGL(); }

	void SetOLOn(bool value)
		{ _ol_on = value; updateGL(); }
	//@}
	...
	*** \param tile_index An ID (range: {0, width * height - 1}) of the tile used to determine the row or column
	***        upon which to perform the operation.
	***
	*** \note This feature is accessed by right-clicking on the map. It could be used elsewhere if the proper
	...
	**/
	...
	//! \brief A list which contains a pointer to each sprite, I use list which is because of its efficient.
...
private:
	...
	***
	*** This data exists only in the editor and is not a part of the map file nor the game.
	*** It acts similar to an actual tile layer as far as drawing is concerned.
	**/
