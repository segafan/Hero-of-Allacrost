...
BattleEffect::BattleEffect(BattleActor* actor) :
	_effect_name(),
	_effect_actor(actor)
{
	if (actor == NULL) {
...
StatusEffect::StatusEffect(GLOBAL_STATUS type, GLOBAL_INTENSITY intensity, BattleActor* actor) :
	BattleEffect(actor),
	_status_effect(type, intensity),
	_opposite_status_type(GLOBAL_STATUS_INVALID),
...
bool StatusEffect::Load(ReadScriptDescriptor& script_file) {
	_effect_name = script_file.ReadUString("name");
	_duration_timer.SetDuration(script_file.ReadUInt("duration"));
	_opposite_status_type = static_cast<GLOBAL_STATUS>(script_file.ReadInt("opposite_effect"));
	...
	_duration_timer.Reset();
	_duration_timer.Run();
	_icon_image = BattleMode::CurrentInstance()->GetMedia().GetStatusIcon(_status_effect.GetType(), _status_effect.GetIntensity());
	return true;
}



void StatusEffect::CallApplyFunction() const {
	if (_apply_function == NULL)
		return;
...
void StatusEffect::CallUpdateFunction() const {
	if (_update_function == NULL)
		return;

	ScriptCallFunction<void>(*_update_function, this);
}



void StatusEffect::CallRemoveFunction() const {
	if (_remove_function == NULL)
		return;

	ScriptCallFunction<void>(*_remove_function, this);
}



void StatusEffect::SetIntensity(hoa_global::GLOBAL_INTENSITY intensity) {
	...
	bool changed = (_status_effect.GetIntensity() == intensity);
	_status_effect.SetIntensity(intensity);
...
bool StatusEffect::IncrementIntensity(uint8 amount) {
	GLOBAL_INTENSITY old_intensity = _status_effect.GetIntensity();
	_status_effect.IncrementIntensity(amount);
	bool changed = (old_intensity == _status_effect.GetIntensity());
...
bool StatusEffect::DecrementIntensity(uint8 amount) {
	GLOBAL_INTENSITY old_intensity = _status_effect.GetIntensity();
	_status_effect.DecrementIntensity(amount);
	bool changed = (old_intensity == _status_effect.GetIntensity());
...
void StatusEffect::_UpdateDurationTimer() {
	_duration_timer.Update();
	if (_duration_timer.IsFinished() == true) {
		DecrementIntensity(1);
	}
}

////////////////////////////////////////////////////////////////////////////////
// StaticStatusEffect class
////////////////////////////////////////////////////////////////////////////////

void StaticStatusEffect::Update() {
	_intensity_changed = false;
	_UpdateDurationTimer();

	if (IsIntensityChanged() == true) {
		if (GetIntensity() != GLOBAL_INTENSITY_NEUTRAL) {
			CallUpdateFunction();
		}
	}
}
...
bool EffectsSupervisor::IsOppositeStatusActive(GLOBAL_STATUS status) {
	for (map<GLOBAL_STATUS, StatusEffect*>::iterator i = _active_status_effects.begin(); i != _active_status_effects.end(); i++) {
		if (i->second->GetOppositeStatusType() == status) {
			return true;
		}
	}
	...
	GLOBAL_STATUS& new_status, GLOBAL_INTENSITY& new_intensity)
{
	...
	StatusEffect* active_effect = NULL;
	...
	for (map<GLOBAL_STATUS, StatusEffect*>::iterator i = _active_status_effects.begin(); i != _active_status_effects.end(); i++) {
		...
		else if (i->second->GetOppositeStatusType() == status) {
...
void EffectsSupervisor::_CreateNewStatus(GLOBAL_STATUS status, GLOBAL_INTENSITY intensity) {
	...
	StatusEffect* new_effect = NULL;

	// --- Make sure that a table entry exists for this status effect
	uint32 table_id = static_cast<uint32>(status);
	...
	string class_type = script_file.ReadString("class_type");
	if (class_type == "StaticStatusEffect") {
	...
	new_effect->CallApplyFunction();
}



void EffectsSupervisor::_RemoveStatus(StatusEffect* status_effect) {
