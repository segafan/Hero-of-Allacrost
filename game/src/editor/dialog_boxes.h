...
public:
	...
	***
	*** This class is used in two instances. For presenting a dialog to the user to (1) create a new map or (2) modify the properties
	*** of an already existing map (such as height, width, or tilesets loaded in the bottom portion of the editor). For case #1, the
	*** parameter prop is false, and for case #2, it is true.
	**/
...
/** ****************************************************************************
*** \brief A dialog box that allows the user to declare what music files to load for this map
***
*** This only defines the music files that the map should load. It does not define
*** when or how each file should be played. It populates two lists of music files:
*** one containing the music already chosen for the map, and the other with all the
*** remaining music files that are not chosen. Each file is mutually exclusive and
*** is only found in one list or the other, preventing the user from declaring to use
*** the same music file more than once.
*** ***************************************************************************/
...
***
*** This simple dialog allows the user to create a new context by inheriting from
*** one of the existing ones. Inheriting an existing context simply "copies" all
*** tiles over into the new context. This makes it easy to tell which tiles need
*** to be switched. For example, replacing the tiles making up an exterior of a
*** house or other structure with interior ones. Contexts are given a name identifier
*** to help the map designer manage their contexts, but this information isn't displayed
*** or otherwise presented in the game.
*** ***************************************************************************/
...
public:
	...
	**/
	...
	//! \name Class member accessor functions
	//@{
	//! \brief Returns the context name entered into the line edit widget by the user.
